...
class UserBillingService {
  ...
  async createCheckoutSession({
    ...
    if (pageCount) {
      const variantId = product.plans.find(plan => plan.id === planId)?.lineItems[0]?.id;
      if (variantId) {

        let finalQuantity = pageCount;

        if (product.id === 'free' && finalQuantity < 5) {
          finalQuantity = 5;
        }

        variantQuantities.push({ variantId, quantity: finalQuantity });
      }
    }
